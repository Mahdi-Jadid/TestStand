...
void Stand_PID::pid_logic(Stand_ESC* stand_esc, Stand_ACS* stand_acs, Stand_Loadcell* stand_loadcell) {
    ...
    if (preSeek) {
      ...
      if (current_windowed_average < (  current_setpoint - 0.5))  // 0.5 A below target
        ...
        // we're close enough, switch to PID fine control
         preSeek = false;
        // initialize PID   current_output around current angle
  ...
  // Penalty: only when locked and above   current_setpoint
